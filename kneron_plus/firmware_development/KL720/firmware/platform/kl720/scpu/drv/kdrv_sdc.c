...
static kdrv_status_t kdrv_sdc_set_bus_width(kdrv_sdc_res_t *dev, uint8_t width)
{
    ...
    if (card_info->card_type == MEMORY_CARD_TYPE_MMC) {
        if (card_info->csd_mmc.spec_vers < 4) {
            //sdc_dbg_printf(" MMC: Change width not allowed for version less than MMC4.1.\n");
            return KDRV_STATUS_OK;
        }
        sdc_dbg_printf("kdrv_sdc_set_bus_width=%d\n",width);
        /**
        ...
        case 1:
            if (!(card_info->scr.sd_bus_widths & SDHCI_SCR_SUPPORT_1BIT_BUS)) {
                //sdc_dbg_printf(" ERR## ... SD: 1 bit width not support by this card.\n");
                return KDRV_STATUS_SDC_BUS_WIDTH_NOT_SUPPORT;
            }
        ...
        case 4:
            if (!(card_info->scr.sd_bus_widths & SDHCI_SCR_SUPPORT_4BIT_BUS)) {
                //sdc_dbg_printf(" ERR## ... SD: 4 bit width not support by this card.\n");
                return KDRV_STATUS_SDC_BUS_WIDTH_NOT_SUPPORT;
            }
            wdth = SDHCI_4BIT_BUS_WIDTH;
            break;
        default:
            //sdc_dbg_printf(" ERR## ... SD: Not supported bus witdth %d.\n", width);
            return KDRV_STATUS_SDC_BUS_WIDTH_NOT_SUPPORT;
        }

        if (kdrv_sdc_ops_app_set_bus_width(dev, wdth)) {
            //sdc_dbg_printf(" ERR## ... SD: Set Bus Width %d failed !\n", width);
    ...
    case 8:
        ...
        break;
    default:
        sdc_dbg_printf(" Unsupport bus width %d for HW register setting.\n", width);
        break;
    }
    sdc_dbg_printf(" Set Bus width %d bit(s), Host Control Register: 0x%x.\n", width, sdc_reg->hcreg);
    return KDRV_STATUS_OK;
}
...
static kdrv_status_t kdrv_sdc_execute_tuning(kdrv_sdc_res_t *dev, uint32_t try)
{
    ...
    if (!((sdc_reg->host_ctl2 & SDHCI_SDR104) && (sdc_reg->host_ctl2 & SDHCI_18V_SIGNAL))) {
        sdc_dbg_printf(" ERR## ... Tuning only require for SDR104 mode.\n");
        return KDRV_STATUS_ERROR;
    }
...
retune:
    ...
    do {
        ...
        do {
            ...
            if (total_time > 1000 * 1000) {
                sdc_dbg_printf(" ERR##: Tuning wait for BUFFER READ READY timeout.\n");
    ...
    if (sdc_reg->host_ctl2 & SDHCI_SMPL_CLCK_SELECT) {
        ...
        sdc_dbg_printf("Tuning Complete(0x%08x), SD Delay : 0x%x.\n", sdc_reg->vendor_reg4, delay);

        if (!(sdc_reg->vendor_reg4 & (1 << delay))) {
            sdc_dbg_printf("SD Delay does not match tuning record(0x%08x).\n", sdc_reg->vendor_reg4);

            if (cnt++ < 3) {
                goto retune;
            } else {
                sdc_dbg_printf("Re-Tuning Failed.\n");
                card_info->flow_set.use_dma = dma;
                return KDRV_STATUS_ERROR;
            }
        }
    } else {
        sdc_dbg_printf("Tuning Failed.\n");
        card_info->flow_set.use_dma = dma;
        return KDRV_STATUS_ERROR;
    }
...
static void kdrv_sdc_set_sd_clock(kdrv_sdc_res_t *dev, uint32_t clock)
{
    ...
    //div=3;
    sdc_dbg_printf(" SDLK Freq select, Div: %d, Clock: (%d/%d) Hz, ", div, (host->max_clk / div), clock);

    clk = div << SDHCI_CLK_CTRL_LOW_CLK_SEL_SHIFT;
    clk |= SDHCI_CLK_CTRL_INTERNALCLK_EN;
    sdc_dbg_printf("sdc_reg->clk_ctl: 0x%x\n", sdc_reg->clk_ctl);
    sdc_reg->clk_ctl = clk;
    ...
    while (!(sdc_reg->clk_ctl & SDHCI_CLK_CTRL_INTERNALCLK_STABLE)) {
        if (timeout == 0) {
            ...
            sdc_dbg_printf(" ERR## ... Internal clock never estabilised.\n");
            break;
        }
        timeout--;
        kdrv_delay_us(1000);
    ...
    sdc_reg->clk_ctl = clk;
    sdc_dbg_printf("div=0x%x, clk=0x%x\n",div, clk);
    ...
    if (s_clk < 100000000) {
        ...
        sdc_dbg_printf("sdc_reg->vendor_reg0=0x%x\n",sdc_reg->vendor_reg0);
...
out:
    host->clock = clock;
    sdc_dbg_printf(" Clock Control Register: 0x%04x.\n", sdc_reg->clk_ctl);
}
...
static kdrv_status_t kdrv_sdc_set_power(kdrv_sdc_res_t *dev, int16_t power)
{
    ...
    default:
        err = KDRV_STATUS_SDC_PWR_SET_ERR;
        sdc_dbg_printf(" ERR## ... Voltage value not correct.\n");
    }
...
out:
    host->power = power;
    sdc_dbg_printf(" Power Control Register: 0x%01x.\n", sdc_reg->pwr_ctl);
    return err;
}
...
static kdrv_status_t kdrv_sdc_hw_attr_init(kdrv_sdc_dev_e dev_id)
{
...
#ifdef KDRV_SDC_DBG
    sdc_dbg_printf(" - Base Clock Frequency: Min %d Hz, Max %d Hz.\n", host->min_clk, host->max_clk);
    sdc_dbg_printf(" - Max Block Length: 0x%x bytes.\n", 0x200 << ((sdc_reg->cap_reg >> 16) & 0x3));
    sdc_dbg_printf(" - 8-bit: %s, ADMA2: %s, HS: %s, SDMA: %s, S/R: %s, Voltage: 0x%x,SDR50/104,DDR50: 0x%x.\n\n",
    ...
    if (sdc_reg->hw_attr & 0x1C) {
        ...
        dev->fifo_depth = 8 << 2;
#ifdef KDRV_SDC_DBG
    sdc_dbg_printf(" - FIFO depth %d bytes.\n", dev->fifo_depth);
#endif
...
static void kdrv_sdcx_isr_handle(kdrv_sdc_dev_e dev_id)
{
    ...
    if (sts & SDHCI_INTR_STS_CARD_INTR) {
        ...
        sdc_dbg_printf("INTR: Card Interrupt.\n");
        sdc_reg->intr_en |= SDHCI_INTR_STS_CARD_INTR;
    }
#if (STANDARD_SD==1)
...
#endif
        sdc_dbg_printf("INTR: Card Insert\n");
        card_info->card_insert = 1;
        //cardChanged = true;
    ...
    if (sts & SDHCI_INTR_STS_ERR) {
        ...
        sdc_dbg_printf(" INTR: ERR## &0x%x 0x%x\n", &(sdc_reg->err_sts),sdc_reg->err_sts);
