...
__weak kdrv_status_t kdrv_csi2rx_enable(uint32_t csirx_idx, struct cam_format *format, uint32_t vstr0, uint32_t vster, uint32_t pftr)
{
    return KDRV_STATUS_OK;
}
__weak kdrv_status_t kdrv_csi2rx_start(uint32_t csirx_idx)
{
    return KDRV_STATUS_OK;
}
/* API */
static kmdw_status_t kmdw_cam_open(uint32_t cam_idx)
{
    ...
    kdrv_csi2rx_set_power(cam_idx, 1, 1);
...
static kmdw_status_t kmdw_cam_close(uint32_t cam_idx)
{
    ...
    if(ctx->cam_input_type != IMG_SRC_IN_PORT_MIPI)
        return KMDW_STATUS_OK;
    cam_msg("cam: %d\n", cam_idx);
    kdrv_csi2rx_set_power(cam_idx, 0, 0);
...
static kmdw_status_t kmdw_cam_set_format(uint32_t cam_idx, struct cam_format *format)
{
    ...
    {
        if(cam_ctx[cam_idx].cam_input_type == IMG_SRC_IN_PORT_DPI)
            bpp = 2;
        else
            bpp = 1;
    }
    ...
    if(ctx->cam_input_type == IMG_SRC_IN_PORT_MIPI)
        kdrv_csi2rx_enable( ctx->id, format, 0x05, 0x08, 0x30);
...
static kmdw_status_t kmdw_cam_buffer_init(uint32_t cam_idx)
{
    struct kmdw_cam_context *ctx = &cam_ctx[cam_idx];

    cam_msg("cam %d: size=%d\n", cam_idx, ctx->fmt.sizeimage);

    kdrv_fb_mgr_init(cam_idx, ctx->fmt.sizeimage, MAX_FRAME_BUFFER, TILE_BLOCKS_MAX);

    kdrv_dpi2ahb_buf_init(cam_idx);

    return KMDW_STATUS_OK;
}

static kmdw_status_t kmdw_cam_start_capture(uint32_t cam_idx)
{
    cam_msg("cam: %d\n", cam_idx);
    
    kdrv_csi2rx_start(cam_idx);
    kdrv_dpi2ahb_start(cam_idx);
    return KMDW_STATUS_OK;
}
...
static kmdw_status_t kmdw_cam_set_exp_time(uint32_t cam_idx, uint32_t exp_time)
{
    cam_msg("cam: %d: exp_time %d\n", cam_idx, exp_time);

    return kmdw_sensor_set_exp_time(cam_idx, exp_time);
}
...
static kmdw_status_t kmdw_cam_get_expo(uint32_t cam_idx)
{
    cam_msg("[%s] cam: %d\n", __func__, cam_idx);

    return kmdw_sensor_get_expo(cam_idx);
}

static kmdw_status_t kmdw_cam_set_inc(uint32_t cam_idx, uint32_t enable)
{
    cam_msg("[%s] cam: %d\n", __func__, cam_idx);

    kmdw_sensor_set_inc(cam_idx, enable);

    return KMDW_STATUS_OK;
}

static cam_ops camera_ops = {
    .open               = kmdw_cam_open,
...
{
    if(cam_ctx[cam_idx].cam_input_type == IMG_SRC_IN_PORT_MIPI)
    {
        kmdw_cam_mipi_init(cam_idx);
    }
    else if(cam_ctx[cam_idx].cam_input_type == IMG_SRC_IN_PORT_DPI)
    {
        kmdw_cam_dpi_init(cam_idx);
    }
}

kmdw_status_t kmdw_cam_init(void)
{
    uint32_t cam_id;
    cam_msg("init\n");
    for(cam_id = 0; cam_id < CAM_ID_MAX ; cam_id++)
    {
        ...
        cam_ctx[cam_id].tile_avg_en     = (cam_id)? IMGSRC_1_TILE_AVG: IMGSRC_0_TILE_AVG;
        if(cam_ctx[cam_id].cam_input_type != IMG_SRC_IN_PORT_NONE)
        {
            ...
            kmdw_sensor_register(cam_id);
            kmdw_cam_port_init(cam_id);
            kdrv_dpi2ahb_initialize(cam_id,cam_ctx[cam_id].tile_avg_en);
        }
    }
    kmdw_cam_clock_init();
